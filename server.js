...
app.post("/get-attendance", async (req, res) => {
  ...
  // NDJSON streaming header
  res.setHeader("Content-Type", "application/x-ndjson; charset=utf-8");
  res.setHeader("Cache-Control", "no-cache");
  res.flushHeaders?.();

  // Log visit immediately (non-blocking)
  supabase
    .from("site_visits")
  ...
  addToQueue(async () => {
    try {
      ...
      try {
        ...
        res.write(JSON.stringify({ step: "error", data: "Invalid Credentials" }) + "\n");
        return res.end();
      }

      // 2️⃣ FETCH PARALLEL
      let academic, biometric, latest;
      ...
      try {
        ...
        res.write(JSON.stringify({ step: "error", data: "Failed to fetch attendance" }) + "\n");
        return res.end();
      }
      ...
      // 3️⃣ FETCH LATEST (optional, but good)
      try {
        latest = await fetchLatestAttendance(cookies);
        res.write(JSON.stringify({ step: "latest", data: latest }) + "\n");
      } catch {
        res.write(JSON.stringify({ step: "latest", data: null }) + "\n");
      }

      // 4️⃣ Save to DB (non-blocking)
      supabase
        .from("student_credentials")
        .upsert({
          username,
      ...
      // 5️⃣ send fetched_at + done
      res.write(JSON.stringify({ step: "fetched_at", data: new Date().toISOString() }) + "\n");
      res.write(JSON.stringify({ step: "done", success: true }) + "\n");

      return res.end();

    } catch (err) {
      res.write(JSON.stringify({ step: "error", data: err.message }) + "\n");
      return res.end();
    }
  });
});
